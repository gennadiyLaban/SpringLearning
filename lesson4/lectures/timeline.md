#### 4.1 Основы Spring. REST API с базой данных
* Модель зрелости REST API:
   - Уровень 0 - передача данных без учёта REST принципов
   - Уровень 1 - API использует HTTP методы для определения операций над ресурсами
   - Уровень 2 - API соответствует принципам REST. Использует уникальные URL для каждого ресурса и различные HTTP-методы для выполнения операций над ресурсами. API становиться более предсказуемым и понятным.
   - Уровень 3 - на этом уровне API включает принцип гипермедиа (HATEOAS - Hypermedia as Engine of Application State). Он представляет дополнительную информацию и связи в ответах API, чтобы клиенты могли навигировать по API и определять доступные операции.
