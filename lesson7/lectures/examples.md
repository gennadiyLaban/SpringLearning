...
- `Sinks.many().multicast().onBackpressureBuffer()` - создаёт множество с множество с несколькими подписчиками, которые используют для обработки сигналов с `buffer` для хранения тех сигналов, которые подписчики не успели обработать. 
